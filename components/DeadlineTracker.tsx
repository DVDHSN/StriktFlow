...
const DeadlineTracker: React.FC<DeadlineTrackerProps> = ({ isOpen, onClose, deadlines, onAdd, onDelete, theme }) => {
            ...
            <div className="space-y-4 overflow-y-auto max-h-[calc(100vh-380px)] pr-2 custom-scroll">
              {deadlines.length === 0 && (
                <div className="text-center py-24 italic opacity-20 font-medium">
                  No upcoming exams or deadlines.<br/>Keep your focus sharp.
                </div>
              )}
                ...
                >
                  <div className="flex items-center gap-5">
                    ...
                    <div className="max-w-[140px]">
                      <h4 className="font-bold text-[16px] truncate">{deadline.name}</h4>
                      <p className="text-[11px] font-bold uppercase tracking-widest opacity-30">{new Date(deadline.date).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <span className="block text-2xl font-black tabular-nums leading-none">{calculateDaysLeft(deadline.date)}</span>
